...
class DislikedFactsViewController: UIViewController {
    ...
    private var allDislikedFacts: [Fact] = []
    private var filteredDislikedFacts: [Fact] = []
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        navigationItem.title = "Disliked Facts"
    ...
    private func setupUI() {
        ...
        tableView.register(DislikedFactTableViewCell.self, forCellReuseIdentifier: "DislikedFactCell")
        ...
        private func loadDislikedFacts() {
        ...
        firebaseManager.loadAllFactsForProfile { [weak self] allFacts in
            DispatchQueue.main.async {
                ...
                // Get disliked facts
                self.allDislikedFacts = allFacts.filter { fact in
                    self.firebaseManager.isFactDisliked(fact.id)
                }
    ...
    private func applyFilter() {
        if let selectedFactPack = selectedFactPack {
            ...
            filteredDislikedFacts = allDislikedFacts.filter { fact in
                fact.factPack == selectedFactPack
            }
        } else {
            // Show all disliked facts
            filteredDislikedFacts = allDislikedFacts
        }
    }
...
extension DislikedFactsViewController: UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return filteredDislikedFacts.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "DislikedFactCell", for: indexPath) as! DislikedFactTableViewCell
        let fact = filteredDislikedFacts[indexPath.row]
...
extension DislikedFactsViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        let count = filteredDislikedFacts.count
        if let selectedFactPack = selectedFactPack {
            let packInfo = factPackManager.getFactPackInfo(selectedFactPack)
            return "\(packInfo.name) (\(count))"
        } else {
            return "All Disliked Facts (\(count))"
        }
    }
    
    func tableView(_ tableView: UITableView, titleForFooterInSection section: Int) -> String? {
        if filteredDislikedFacts.isEmpty {
            if let selectedFactPack = selectedFactPack {
                let packInfo = factPackManager.getFactPackInfo(selectedFactPack)
                return "You haven't disliked any facts from \(packInfo.name) yet."
            } else {
                return "You haven't disliked any facts yet."
            }
        }
        return nil
    }
...
class DislikedFactTableViewCell: UITableViewCell {
    ...
    private func setupUI() {
        ...
        // Setup fact pack badge (red for disliked) with dark mode support
        factPackBadge.font = UIFont.systemFont(ofSize: 11, weight: .semibold)
        factPackBadge.textColor = UIColor.white
        factPackBadge.backgroundColor = UIColor.systemRed
